<?php
    ...
    $img = imagecreatefrompng('./bk11_pattern.png');
    $width = imagesx($img);
    $height = imagesy($img);
    ...
    $f = fopen ("bk11_pattern.mac", "w");
    fputs($f, "PatternTiles:\n\t.byte\t");
    $total = count($tilesMap);
    for ($i=0; $i<$total; $i++)
    {
	fputs($f, str_pad(decoct($tilesMap[$i]), 3, '0', STR_PAD_LEFT));
	if ($i%16 != 15) fputs($f, ", "); else 
	  if ($i<($total-1)) fputs($f, "\n\t.byte\t");
    }
    fputs($f, "\n\n");
    ...
    {
	$tile = $tilesArray[$t];
	fputs($f, "PatternTile".str_pad("".$t, 3, "0", STR_PAD_LEFT).":\n");
	fputs($f, "\t.word\t");
	for ($i=0; $i<8; $i++)
	{
