<?php
    ...
    $img = imagecreatefrompng('./mario_buffer.png');
    $width = imagesx($img);
    $height = imagesy($img);
    ...
    $f = fopen ("mario_buffer.mac", "w");
    fputs($f, "TilesMap:\n\t.byte\t");
    $total = count($tilesMap);
    for ($i=0; $i<$total; $i++)
    {
	fputs($f, decoct($tilesMap[$i]));
	if ($i%16 != 15) fputs($f, ", "); else if ($i<($total-1)) fputs($f, "\n\t.byte\t");
    }
    fputs($f, "\n\n");
    ...
    {
	$tile = $tilesArray[$t];
	fputs($f, "TileData".str_pad("".$t, 3, "0", STR_PAD_LEFT).":\n");
	fputs($f, "\t.word\t");
	for ($i=0; $i<32; $i++)
	{
	    fputs($f, decoct($tile[$i]));
	    if ($i<31) fputs($f, ", "); else fputs($f, "\n");
	}
    }
    fputs($f, "\n\n");
